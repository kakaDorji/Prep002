    ...
    <div class="relative py-3 max-w-6xl w-full mx-auto px-4">
      ...
      <div class="relative px-6 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
        ...
        <div class="overflow-x-auto">
          <table v-if="paginatedUsers.length > 0" class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th v-for="header in ['Username','Full Name', 'Designation', 'Prep Location', 'Role', 'BHC No', 'Action']" 
                    :key="header" 
              ...
              <tr v-for="user in paginatedUsers" :key="user.username" class="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button v-if="!user.isEditing" @click="toggleEdit(user)" 
                          class="text-cyan-600 hover:text-cyan-900 mr-2 transition-colors duration-200">Edit</button>
                  <template v-else>
                    <button @click="saveChanges(user)" 
                            class="text-green-600 hover:text-green-900 mr-2 transition-colors duration-200">Save</button>
                    <button @click="cancelEdit(user)" 
                            class="text-red-600 hover:text-red-900 transition-colors duration-200">Cancel</button>
                  </template>
                  <button @click="requestDelete(user)" 
        ...
        <div v-if="successMessage || errorMessage" class="mb-4 max-w-full">
          <div v-if="successMessage" class="bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded shadow-md text-center mb-2">
            {{ successMessage }}
          </div>
          <div v-if="errorMessage" class="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded shadow-md text-center">
            {{ errorMessage }}
          </div>
        </div>

        <!-- Confirm delete message -->
      <div v-if="userToDelete" class="mb-6 max-w-full bg-gray-100 border border-gray-300 text-gray-800 px-6 py-4 rounded shadow-md text-center">
  <p class="mb-4">Are you sure you want to delete user: <strong>{{ userToDelete.username }}</strong>? This cannot be undone.</p>
  <button @click="confirmDelete" class="bg-red-600 text-white px-4 py-2 rounded mr-4 hover:bg-red-700 transition">Yes, Delete</button>
  <button @click="cancelDelete" class="bg-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-400 transition">Cancel</button>
</div>
...
async function fetchUsers() {
  ...
  try {
    ...
    if (data.success && data.users && Array.isArray(data.users)) {
      ...
      throw new Error(data.message || 'Failed to fetch users or unexpected data format.');
    }
  } catch (error) {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      users.value = users.value.filter(u => u.rowindex !== userToDelete.value.rowindex);
...
const filteredUsers = computed(() =>
  users.value.filter(user =>
    ...
    (user.fullname && user.fullname.toLowerCase().includes(searchTerm.value.toLowerCase()))
  )
);
